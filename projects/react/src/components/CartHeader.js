...
export function CartHeader({setup}){
  let {cart,cartQ,onRemoveCartElm} = setup;
  ...
  useEffect(() => {
    if(cnrRef.current){
      cnrRef.current.classList.add('cart-bn-wr_animate');
      setTimeout(()=>{
        cnrRef.current.classList.remove('cart-bn-wr_animate');
      },500);
    }
  },[cartQ]);
  ...
  if(cart){
    for(let entry of Object.entries(cart)){
      ...
      cartList.push(
        <div key={entry[0]} k={entry[0]} className="cart-modal__tr">
          <div className="cart-modal__td">
            <Link to='../card' state={{"id":entry[0]-1}}>
              <div className="cart-modal__img-a">
                <img className={"cart-modal__img" + (imgStatus == "loaded" ? "" : " opy0")} alt=""
                src={entry[1]['cardSrc']} onLoad={handleImageLoaded}/>
              </div>
            </Link>  
          </div>
          <div className="cart-modal__td">
            <div className="cart-modal__h">
              <Link to='../card' state={{"id":entry[0]-1}}>
                <div className="cart-modal__h-a">{entry[1]['name']}</div>
              </Link>  
            </div>
            <div className="cart-modal__txt">{entry[1]['price']}р {entry[1]['quantity']}шт</div>
          </div>
          <div className="cart-modal__td">
            <div className="cart-modal__close" onClick={() => {
              if(cartList.length <= 1) setIsAct(false);
              onRemoveCartElm(entry[0]);}}>
              <div className="cart-modal__close-bn"></div>
            </div>
          </div>
        </div>
      );
  ...
  return (
    <div className={"cart-modal-wr" + (isAct && cartQ > 0 ? " cart-modal-wr_act" : "")}>
      <div ref={cnrRef} className="cart-bn-wr">
        <div className="cart-bn" onClick={() => {if(cartList.length > 0) setIsAct(prv => !prv)}}>
          <div className="cart-bn__quantity">{cartQ}</div>
          <svg height="64px" width="64px" viewBox="0 0 512 512" fill="#fff" >
      ...
      <div className="cart-modal">
        <div className="cart-modal__t-wr">
          <div className="cart-modal__t">
            {cartList}
          </div>  
        </div>
        <div className="cart-modal__btm">
          <Link to='../cart'>
            <div className="cart-modal__btn" onClick={() => setIsAct(false)}>Открыть корзину</div>
          </Link>
          <div className="cart-modal__sum">Итого: {sum}р</div>
        </div>
      </div>
    </div>  
  );
