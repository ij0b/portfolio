...
export function Cart({setup}){
  let {cart,cartQ,onRemoveCartElm,setCart,setCartQ,setPage} = setup;
  let [imgStatus,setImgStatus] = useState({});
  let cnrRef = useRef(null);
  let [isModalBoxAct,setIsModalBoxAct] = useState(false);
  let [isSubmit,setIsSubmit] = useState(false);

  function handleImageLoaded(id) {
    setImgStatus(prv => {return({
      ...prv,
      [id] : "loaded"
    })});
  }
  function handleCartQ2(id,quantity) {
    if(quantity){
      setCart(prv => {return ({
        ...prv, 
        [id] : {
    ...
    let cartQN = cart[id]["quantity"] - quantity;
    setCartQ(prv => prv - cartQN);
  }

  let cartList = [];
  let sum = 0;
  if(cart){
    for(let entry of Object.entries(cart)){
      sum += entry[1]['price'] * entry[1]['quantity'];
      cartList.push(
        <div key={entry[0]} k={entry[0]-1} className="cart__tr">
          <div className="cart__td">
            <Link to='../card' state={{"id":entry[0]-1}}>
              <div className="catalog-card__img-a">
                <span className={"catalog-card__loader" + (imgStatus[entry[0]] == "loaded" ? " opy0" : "")}></span>
                <img className={"catalog-card__img" + (imgStatus[entry[0]] == "loaded" ? "" : " opy0")} alt=""
                  src={entry[1]['cardSrc']} onLoad={() => handleImageLoaded(entry[0])}/>  
              </div>
            </Link>
          </div>
          <div className="cart__td">
            <div className="cart__h">
              <Link to='../card' state={{"id":entry[0]-1}}>
                <div className="cart__h-a">{entry[1]['name']}</div>
              </Link>
            </div>
            <div className="cart__txt">
              {entry[1]['price']}р
            </div>
            <Quantity quantity={entry[1]['quantity']} onQuantity={handleCartQ2} isCart={true} id={entry[0]}/>
          </div>
          <div className="cart__td">
            <div className="cart__close" onClick={()=>onRemoveCartElm(entry[0])}>
              <div className="cart__close-bn"></div>
            </div>
          </div>
        </div>
      );
  ...
  return (
    <main className="main">
      <h1 className="h1 h1_center">Корзина</h1>
      <div className="cart">
        {cartQ ? <>
        <div className="cart__t">
          {cartList}
        </div>  
        <div className="cart__btm">
          <ModalBox isModalBoxAct={isModalBoxAct} setIsModalBoxAct={setIsModalBoxAct}>
            <div className="cart__btn" onClick={handleModalBoxAct}>Оформить заказ</div>
            <div className="cart-box">
              <div className="cart-box__h">Оформить тестовый заказ</div>
              <form className="cart-box__form" method="post">
                <input className="cart-box__inp" type="text" placeholder="Имя"/>
                <input className="cart-box__inp cart-box__inp_read" type="email" 
                  value="test@mail.ru" readOnly="readOnly" onMouseDown={evt => evt.preventDefault()}/>
                <input className="cart-box__inp  cart-box__inp_read" type="text" 
                  value="8(999)999-99-99" readOnly="readOnly" onMouseDown={evt => evt.preventDefault()}/>
                <button className="cart-box__btn" onClick={evt => {evt.preventDefault(); evt.stopPropagation(); handleSubmit();}}>
